...
function SearchPanel({ trips, myReservations, onReserveTrip, onCancelReservation }) {
  ...
  const [filters, setFilters] = useState({ from: '', to: '', date: '', passengers: 1 });
  const reserved = new Map(myReservations.map((r) => [r.tripId, r]));

  const filteredTrips = useMemo(() => {
    if (!filters.date) return [];
    return trips.filter((t) => {
      const [fromCity = '', toCity = ''] = String(t.route || '').split('→').map((x) => norm(x));
      return (
        t.mode !== 'parcel-only' &&
        tripDateValue(t) === filters.date &&
        fromCity === norm(filters.from) &&
        toCity === norm(filters.to) &&
        (t.status ? t.status === 'active' : true) &&
        Number(t.seats) >= Number(filters.passengers)
      );
    });
  }, [trips, filters]);
  ...
  const onSubmit = (e) => {
    ...
    const passengers = Math.max(1, Math.min(4, Number(data.passengers || 1)));
    setLoading(true);
    setTimeout(() => {
      setFilters({ from: norm(data.from), to: norm(data.to), date: data.date, passengers });
      setLoading(false);
    }, 120);
  };

  return <section className="card"><div className="card-title-row"><h2>Caută cursă</h2><span className="status">Pasager</span></div><form className="form-grid" onSubmit={onSubmit}><label>Oraș plecare<input required name="from" /></label><label>Oraș destinație<input required name="to" /></label><label>Data<input required type="date" name="date" /></label><label>Număr pasageri<input required type="number" min="1" max="4" defaultValue="1" name="passengers" /></label><div className="full"><button className="btn">Caută curse</button></div></form>{loading ? <p className="meta">Se caută curse...</p> : <TripsList trips={filteredTrips} reservedMap={reserved} onReserveTrip={onReserveTrip} onCancelReservation={onCancelReservation} selectedDate={filters.date} type="passenger" passengersSelected={filters.passengers} />}</section>;
}
...
function TripsList({ trips, reservedMap, onReserveTrip, onReserveCourier, onCancelReservation, selectedDate, type, passengersSelected = 1 }) {
