...
function SearchPanel({ trips, myReservations, onReserveTrip, onCancelReservation }) {
  ...
  const filteredTrips = useMemo(() => {
    ...
    return trips.filter((t) => t.mode !== 'parcel-only' && t.date === selectedDate);
  }, [trips, selectedDate]);
  ...
  return (
    <section className="card">
      ...
      <form className="form-grid" onSubmit={onSubmit}>
        <label>Oraș plecare<input name="from" /></label>
        <label>Oraș destinație<input name="to" /></label>
        <label>Data<input required type="date" name="date" /></label>
        <label>Număr pasageri<input required type="number" min="1" max="4" defaultValue="1" name="passengers" /></label>
        <div className="full"><button className="btn">Caută curse</button></div>
      </form>
      {loading ? <p className="meta">Se caută curse...</p> : <TripsList trips={filteredTrips} reservedMap={reserved} onReserveTrip={onReserveTrip} onCancelReservation={onCancelReservation} selectedDate={selectedDate} />}
    </section>
  );
}
...
function CourierPanel({ trips }) { const courierTrips = trips.filter((t) => t.mode !== 'passengers'); return <section className="card"><div className="card-title-row"><h2>Curier</h2><span className="status warning">Colete</span></div><TripsList trips={courierTrips} reservedMap={new Map()} hideActions selectedDate="" /></section>; }

function TripsList({ trips, reservedMap, onReserveTrip, onCancelReservation, hideActions = false, selectedDate }) {
  ...
  return <div className="list">{trips.map((t) => { const res = reservedMap.get(t.id); const full = t.seats <= 0; const maxAllowed = Math.min(4, t.seats || 0); return <article key={t.id} className="card trip-card"><h3>{t.route}</h3><div className="trip-meta"><span>Șofer: {t.driver}</span><span>Plecare: {t.time}</span></div><div className="trip-meta"><span>Preț: {t.price}</span><span>{full ? 'Cursă plină' : `Locuri: ${t.seats}`}</span></div>{(t.mode === 'parcel-only' || t.mode === 'mixed') && !!t.parcelSize && <p className="meta">📦 {t.parcelSize}</p>}{!hideActions && <div className="action-row">{!res && !full && <div className="row-2"><select id={`p-${t.id}`} defaultValue="1">{Array.from({ length: maxAllowed }, (_, i) => i + 1).map((n) => <option key={n} value={n}>{n} pasager{i === 0 ? '' : 'i'}</option>)}</select><button className="btn reserve-btn" onClick={() => onReserveTrip(t.id, Number(document.getElementById(`p-${t.id}`).value || 1))}>Rezervă cursa</button></div>}{res && <button className="btn subtle-btn" onClick={() => onCancelReservation(t.id)}>Anulează rezervarea</button>}{!res && full && <button className="btn subtle-btn" disabled>Cursă plină</button>}</div>}</article>; })}</div>;
}
